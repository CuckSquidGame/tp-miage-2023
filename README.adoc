...
Le code n'est pas structuré en couches : Il est séparé en fichiers tous situés dans le même package.
Cela peut poser différents problèmes comme par exemple :
	- le code peut être difficile à maintenir car il n'est pas évident de comprendre comment il est structuré.
	- il peut être difficile d'ajouter des fonctionnalités (refonte parfois nécessaire)

*Où se trouve le code métier (voir la règle de gestion RG 1) ?*

Le code métier est situé dans la classe "TodoListController".

*Cette règle est-elle facilement testable par un test unitaire ?*

La règle est facilement testable :
Il suffit de s'assurer que la chaine de caractère contient ou non "[LATE!]" (test de type "assert.... containsString()...")
...
- "docker run " lance un conteneur Docker à partir d'une image existante.
- l'option "-it" permet de le lancer en mode interactif.
- l'option ""-e POSTGRES_PASSWORD=password" définit le mot de passe de la base de données Postgre dans une variable d'environnement.
- l'option "p 5432:5432" associe les ports 5432 du conteneur et de l'hôte.
- "postgres" correspond à l'image docker utilisée.
* Compléter le code manquant dans la méthode `TodoListController.createTodoItem()`

*Pourquoi `todoItemRepository` est-il `null` ? Quelle est la meilleure façon de l'injecter ?*

`todoItemRepository` est `null` car c'est une interface, qui ne peut donc pas être instanciée.
Je l'ai injecté dans le constructeur "TodoListController" grâce à @Inject.
...
L'architecture hexagonale est un modèle d'architecture logicielle qui permet une bonne testabilité et évolutivité de l'application.
Cette architecture isole les aspects fonctionnels et les aspects techniques.
Elle est constituée du Domaine (qui correspond à la logique métier) et des adapters, qui peuvent solliciter le Domaine en utilisant des ports in (implémentés
par les adapters), ou qui peuvent être utilisés indirectement par le domaine via des ports out (implémentés par le Domaine).
L'architecture hexagonale respecte l'inversion de dépendance, et favorise les tests en isolation : tests unitaires coté Domaine, et tests d'intégration coté
adapters.
...
Pour les adapteurs, les tests qu'il faudrait écrire sont des tests d'intégration.
